...
import { removeAuthenticatorAppService } from "@/lib/services";
...
export const SettingsRemoveAuthenticatorAppDialog = () => {
	...
	const onSubmit = async (data: z.infer<typeof FormSchema>) => {
		// const { pin } = data;
		const toastId = toast.loading("Removing authenticator app...", {
			id: "remove-authenticator-app",
		});

		try {
			setIsLoading(true);

			await removeAuthenticatorAppService({
			...
			toast.error(`Failed to remove authenticator app: ${error.message}`, {
				id: toastId,
			});
		} finally {
			setIsLoading(false);
